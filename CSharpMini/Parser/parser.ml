...
module Expression = struct
  ...
  and unarExpr input =
    choice
      [
        ...
        ( token "-" >> lexeme primExpr >>= fun x ->
          return (NumExpr (Sub (Value (VInt 0), x))) );
        primExpr;
      ]
      input
  ...
  and arrayAccess input =
    ( callMethod <|> identifier >>= fun aName ->
      many1 (brackets expression) >>= fun aIndexes ->
      return (ArrayAccess (aName, aIndexes)) )
      input

  and accessByPoint input =
    ( arrayAccess <|> callMethod <|> identifier >>= fun pName ->
      token "." >> lexeme expression >>= fun pCall ->
      return (AccessByPoint (pName, pCall)) )
      input

  and splitByComma input = sep_by expression (token ",") input

  and callMethod input =
    ( identifier >>= fun mName ->
      token "(" >> splitByComma >>= fun mArgs ->
      token ")" >> return (CallMethod (mName, mArgs)) )
      input
end
